...
class BlobGAN(BaseModule):
    ...
    def gen(self, z=None, layout=None, ema=False, norm_img=False, ret_layout=False, ret_latents=False, noise=None,
        ...
        # interpolate scores_pyramid and feature grid in layout
        G = self.generator_ema if ema else self.generator
        layout['feature_grid'] = F.interpolate(layout['feature_grid'], size=(G.size_in,int(self.aspect_ratio*G.size_in)))
        layout['scores_pyramid'] = {k: F.interpolate(v, size=(k,int(self.aspect_ratio*k))) for k, v in layout['scores_pyramid'].items()}
        gen_input = {
                ...
                if log_images and is_rank_zero():
                    if self.accumulate and self.n_ema_sample:
                        with torch.no_grad():
                            ...
                            imgs['gen_imgs_ema'] = self.gen(z, ema=True, viz=True, no_jitter=True)
