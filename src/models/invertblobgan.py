...
class Lossλs:
    real_LPIPS: float = 1.
    real_MSE: float = 1.
    real_decision: float = 1.
    fake_LPIPS: float = 1.
    fake_MSE: float = 1.
    fake_decision: float = 1.
...
class BlobGANInverter(BaseModule):
    ...
    def __post_init__(self):
        ...
        if hasattr(self.generator_pretrained,'log_dir') and self.generator_pretrained.log_dir is None:
            self.generator_pretrained.log_dir = 'logs/'
        if type(self.generator_pretrained) is DictConfig:
            self.generator = load_pretrained_weights('BlobGAN', self.generator_pretrained, BlobGAN(**cfg), strict=False)
        else:
            self.generator = BlobGAN.load_from_checkpoint(self.generator_pretrained, strict=False)
        ...
        self.inverter = networks.get_network(**self.inverter,
                                             d_out=self.generator.layout_net_ema.mlp[-1].weight.shape[1])
        #self.inverter = networks.get_network(**self.inverter,
        #                                     d_out=self.generator.noise_dim)
        #self.inverter = networks.StyleGANEncoderNet(512, #self.generator.resolution,
        #                                            w_space_dim=self.generator.layout_net_ema.mlp[-1].weight.shape[1],
        #                                            use_bn=False)                                   
    ...
    def shared_step(self, batch: Tuple[Tensor, dict], batch_idx: int,
        ...
        log_images = run_at_step(batch_idx, self.log_images_every_n_steps)

        z = torch.randn(len(batch_real), self.generator.noise_dim).type_as(batch_real).to(self.device)
