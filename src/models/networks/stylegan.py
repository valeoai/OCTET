...
class StyleGANDiscriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], discriminate_stddev=True, in_channels=3,
                 blur_input=False, blur_kernel_size=3, blur_sigma=1, d_out=1, aspect_ratio = 1):
        super().__init__()
        ...
        self.final_linear = nn.Sequential(
            EqualLinear(int(channels[4] * 4 * 4 * self.aspect_ratio), channels[4], activation="fused_lrelu"),
    ...
    def forward(self, input):
        ...
        if self.discriminate_stddev:
            if batch % self.stddev_group == 0 or batch < self.stddev_group:
