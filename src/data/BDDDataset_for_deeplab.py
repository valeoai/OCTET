...
class BDDDataset(torch.utils.data.Dataset):
    def __init__(self, opt, for_metrics):
        ...
        opt.semantic_nc = 18 # label_nc + unknown
    ...
    def __getitem__(self, idx):
        ...
        label = label * 255
        label += 1.0
        label = torch.where(label > 255, torch.FloatTensor([0.0]), label)
        return {"image": image, "label": label, "name": self.images[idx]}

    def list_images(self):
        mode = "val" if self.opt.phase == "test" or self.for_metrics else "train"
        images = []
        path_img = os.path.join(self.opt.dataroot, "seg", "images", mode)

        for item in sorted(os.listdir(path_img)):
            if item.find("resized") == -1:
                images.append(item)

        labels = []
        path_lab = os.path.join(self.opt.dataroot, "seg", "labels", mode)
        ...
        for i in range(len(images)):
            assert images[i].replace(".jpg", "") == labels[i].replace("_train_id.png", ""),\
    ...
    def transforms(self, image, label):
        ...
        image = TR.functional.resize(image, self.opt.load_size, Image.BICUBIC)
        label = TR.functional.resize(label, self.opt.load_size, Image.NEAREST)
        #centercrop
        #image = TR.functional.center_crop(image, self.opt.load_size)
        #label = TR.functional.center_crop(label, self.opt.load_size)
        # flip
